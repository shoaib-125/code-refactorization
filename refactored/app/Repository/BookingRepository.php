...
class BookingRepository extends BaseRepository
{
    ...
    /**
     * New-Comment
     * getUsersJobs will return the user jobs based on conditions
     * @param $user
     * @param $conditions
     * @param $page
     * @return array
     */
    public function getUsersJobs($user, $conditions, $page = 0)
    {
        $jobs = [];
        if ($user && $user->is('customer')) {
            $jobs = Job::getCustomerJobs($user->id);
        } elseif ($user && $user->is('translator')) {
            $jobs = Job::getTranslatorJobs($user->id);
        }

        if ($jobs && count($conditions) > 0)
            $jobs->where($conditions);

        $count = $jobs->count();

        $jobs['emergencyJobs'] = $jobs->whereImmediate('yes')->orderBy('due', 'asc');
        $jobs['normalJobs'] = $jobs->whereNot('immediate','yes')->orderBy('due', 'asc');

        if ($page > 0) {
            $jobs['emergencyJobs']->offset($page)->limit(15);
            $jobs['normalJobs']->offset($page)->limit(15);
        }
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            /**
             * New-Comment
             * Laravel request is used to validate the fields
             */
            ...
            /**
             * new-Comment
             * Event fired against new job created
             */
    ...
    public function storeJobEmail($data)
    {
        ...
        /**
         * New-Comment
         * Mail is sent in event listener
         */
    ...
    public function jobEnd($post_data = array())
    {
        ...
        /**
         * New-Comment
         * Mails are sent in event listener
         */
...
//        if (in_array($data['status'], ['pending', 'assigned']) && date('Y-m-d H:i:s') <= $job->due) {
        ...
        if ($data['status'] == 'pending') {
            ...
            */
            Event::fire(new ChangeTimedoutStatus($data));
            ...
            $job->save();
            Event::fire(new ChangeTimedoutStatus($job, $data));
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout', 'completed'])) {
        ...
        if ($data['status'] == 'completed') {
            ...
            /**
             * New-Comment
             * Mail sent on event listener
             */
            Event::fire(new ChangeStartedStatus($job, $data));
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout', 'assigned'])) {
        ...
        Event::fire(new changePendingStatus($job, $data));
    ...
    private function changeAssignedStatus($job, $data)
    {
        if (in_array($data['status'], ['withdrawbefore24', 'withdrawafter24', 'timedout'])) {
            ...
            if (in_array($data['status'], ['withdrawbefore24', 'withdrawafter24'])) {
                /**
                 * New-Comment
                 * Mail is sent on Event listener
                 */
                Event::fire(new ChangeAssignedStatus($job, $data));
    ...
    public function acceptJob($job_id, $user)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($job_id, $user->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($user->id, $job_id)) {
                 ...
                 * Mail sent on event listener
                 */
                Event::fire(new AcceptJob($job, $data));
